...
"Jump to previous/next command" sounds like a convenience. It is actually the **focus model**, and three later features consume it: fold/unfold ([phase-8-document-engine.md](phase-8-document-engine.md)), the raw toggle and per-block export ([phase-10-document-view.md](phase-10-document-view.md)), and copy, which currently has no keyboard path at all.
