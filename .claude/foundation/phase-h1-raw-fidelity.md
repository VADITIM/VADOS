...
- **`rowRuns` counts cells and indexes characters, and they are not the same number.** It first took the cell index as the character index, which is right for everything one cell wide and wrong for every row that is not: a CJK glyph or an emoji is one character over two cells, a combining mark is two characters in one cell, and a run indexed in cells lands off its own text. The fix is to walk `getChars().length` per cell and skip the zero-width halves, which is what `translateToString` does to build the text the runs are measured against. It moved to `ansi.js` with the rest of the colour mapping to get a self-check that does not need a terminal — a fake buffer line is four properties.
...
- ~~**`inverse` and the theme.**~~ Settled, and it is both: *which* two values are being swapped is a fact about the cell and is resolved at snapshot time, while the side the cell left unset stays the token `var(--surface-base)` / `var(--text)` and resolves at paint time. So a reverse-video run follows a theme change the way the raw view does, and the snapshot pass stops calling `getComputedStyle` per inverse cell.
