...
- **Reading a block back out of xterm resumes; it does not restart.** The block renderer takes its text from xterm's screen buffer rather than from the byte stream, and re-reading every row of a block on every chunk is O(n²) over one command at a `translateToString` and a regex per row. `git --no-pager diff` is the case that proved it: thousands of rows over dozens of chunks, seconds behind a raw terminal that has nothing to re-read at all. The read resumes from where the last one committed, which is two rows behind the cursor — the join between two rows is decided by looking at the row after them, so the tail is always re-read. A repaint (the block got *shorter*) or a reflow (a resize) invalidates the whole cache, because in both cases rows already read have been rewritten. This is the same rule as the incremental-parse condition below, one level down: never re-read from the top of the block.
...
- **Incremental markdown parsing is allowed only under all three of these.** The original rule here was "never parse a streaming block"; [../foundation/phase-8-document-engine.md](../foundation/phase-8-document-engine.md) needs live rendering, so the ban is replaced by conditions rather than quietly dropped:
